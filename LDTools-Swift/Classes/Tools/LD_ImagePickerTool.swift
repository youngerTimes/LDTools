...
/// 本地图片选择器
public class LD_ImagePickerTool: NSObject{
    public typealias LD_ImagePickerClouse = ((UIImage)->Void)?
    public typealias LD_ImagePickerMutiClouse = (([UIImage],[Any])->Void)?
    ...
    private var clipFrame:CGRect{
        get{
            ...
            clipFrame.origin = CGPoint(x: (LD_ScreenW - clipSize.width)/2.0, y: (LD_ScreenH - clipSize.width)/2.0)
            return clipFrame
        }
    }

    public static let `default`:LD_ImagePickerTool = {
        let center = LD_ImagePickerTool()
        return center
    }()
    ...
    ///   - clouse: 回调
    ///   - clipSize: 裁剪大小
    public func profile(_ clouse:LD_ImagePickerClouse, needClip:Bool = true){
        self.clouse = clouse
        self.needClip = needClip
        self.chooseMethodType = .profile

        if needClip {
            self.clipSize = CGSize(width: LD_ScreenW, height: LD_ScreenW)
        }
    ...
    ///   - clouse: 回调
    ///   - clipSize: 如果需要裁剪，设置
    public func singleImage(_ clouse:LD_ImagePickerClouse, clipSize:CGSize? = nil){
        self.clouse = clouse
    ...
    ///   - clouse: 回调
    ///   - max: 最大的选择数量
    public func multiImage(_ clouse:LD_ImagePickerMutiClouse,max:NSInteger,selectAsstes:NSMutableArray? = nil){
...
extension LD_ImagePickerTool:TZImagePickerControllerDelegate{
...
extension LD_ImagePickerTool:UIImagePickerControllerDelegate{
...
//                    LD_currentViewController().present(corpperVC, animated: true, completion: nil)
//                }
...
//extension LD_ImagePickerTool:ImageCropperDelegate{
...
extension LD_ImagePickerTool:UINavigationControllerDelegate{}
