...
public class LD_CustomDatePickerView: UIView {
    ...
    private var resignYear = Date().ld_nowYear() //辞职年
    private var resignMonth = Date().ld_nowMonth() //辞职月份
    private var inJobYear = Date().ld_nowYear() //入职年
    private var inJobMonth = Date().ld_nowMonth() //入职月份
    ...
    public var selectDay:Int = Date().ld_nowDay()
    public var selectHour:Int = Date().ld_nowHour()
    ...
    override init(frame: CGRect) {
        ...
        pickerView.snp.makeConstraints { (make) in
            ...
            make.bottom.equalToSuperview().offset(-20 * LD_RateW)
    ...
    @objc func hiden(){
        ...
        UIView.animate(withDuration: 0.3, animations: {
            self.centerView.frame = CGRect(x: 0, y: LD_ScreenH, width: LD_ScreenW, height: 325 * LD_RateW)
    ...
    public func showGraduate(inSY:NSInteger,inSM:NSInteger,outSY:NSInteger,outSM:NSInteger,vc:UIViewController,callback:@escaping CallbackSelect,hiden:@escaping CallbackHidden){
        ...
        if inSY == 0{
            ...
            for i in inSY...date.ld_nowYear() + 8{
                allYears.append(i)
            }
        }
    ...
    public func changesSpearatorLine(){
        for view in pickerView.subviews {
            if view.frame.size.height <= 1 {
                ...
                view.frame.origin.x = view.frame.origin.x - 50 * LD_RateW
                view.frame.size.width = LD_ScreenW - 100 * LD_RateW
            }
        }
    }
...
extension LD_CustomDatePickerView:UIPickerViewDataSource{
        ...
        if component == 1 {
            switch datePickerSubType {
                ...
                case .Resign:
                    ...
                    }else if selectYear == date.ld_nowYear(){
                        return date.ld_nowMonth()
                ...
                case .enrol:
                    if inSchoolYear == outSchoolYear && inSchoolMonth == outSchoolMonth{
                        return  date.ld_allMonths().count - outSchoolMonth + 1
                    }else if inSchoolYear == date.ld_nowYear(){
                        return date.ld_nowMonth()
                    }
                    else{
                        return date.ld_allMonths().count
                ...
                case .graduate:
                    if selectYear == inSchoolYear{
                        return date.ld_allMonths().count - inSchoolMonth + 1
                    }
                    else{
                        return date.ld_allMonths().count
                }
    ...
    public func pickerView(_ pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusing view: UIView?) -> UIView {
        ...
        if component == 1 {
            if datePickerSubType == .Resign && selectYear == inJobYear && selectYear != date.ld_nowYear(){
                label.text = String(format: "%02ld月", date.ld_allMonths()[inJobMonth - 1 + row])
            }else if datePickerSubType == .Resign && selectYear == inJobYear && selectYear == date.ld_nowYear(){
                label.text = String(format: "%02ld月", date.ld_allMonths()[inJobMonth - 1 + row])
            }else if datePickerSubType == .graduate && selectYear == inSchoolYear{
                label.text = String(format: "%02ld月", date.ld_allMonths()[inSchoolMonth - 1 + row])
            }else if datePickerSubType == .today && selectYear == date.ld_nowYear(){
                label.text = String(format: "%02ld月", date.ld_allMonths()[date.ld_nowMonth() - 1 + row])
            }else{
                label.text = String(format: "%02ld月", date.ld_allMonths()[row])
            }
        }
        if component == 2 {
            if datePickerSubType == .today{
                label.text = String(format: "%02ld日", row + date.ld_nowDay())
            }else{
                label.text = String(format: "%02ld日", date.ld_allDays()[row])
            }
        }
        if component == 3{
            if limitToday && date.ld_nowYear() == selectYear && date.ld_nowMonth() == selectMonth && date.ld_nowDay() == selectDay{
...
extension LD_CustomDatePickerView:UIPickerViewDelegate{
    public func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if component == 1 {
            component_1_row = row
            if datePickerSubType == .Resign && selectYear == inJobYear && selectYear != date.ld_nowYear(){
                selectMonth = date.ld_allMonths()[inJobMonth - 1 + row]
            }else if datePickerSubType == .graduate && selectYear == inSchoolYear{
                selectMonth = date.ld_allMonths()[inSchoolMonth - 1 + row]
            }else if datePickerSubType == .today && selectYear == date.ld_nowYear(){
                selectMonth = date.ld_nowMonth() + row
            }
            else{
        ...
        if component == 2 {
            ...
            if limitToday && date.ld_nowYear() == selectYear && date.ld_nowMonth() == selectMonth{
                selectDay = date.ld_allDays()[row] + date.ld_nowDay() - 1
            }else{
