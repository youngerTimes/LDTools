...
public extension CALayer {
    ...
    ///   - option: 动画的时间节奏控制 方式
    func ld_animationMovePoint(to endPoint: CGPoint,
                               duration: TimeInterval,
                               delay: TimeInterval = 0,
                               repeatNumber: Float = 1,
                               removedOnCompletion: Bool = false,
                               option: CAMediaTimingFunctionName = .default) {
        ld_baseBasicAnimation(keyPath: "position", startValue: position, endValue: endPoint, duration: duration, delay: delay, repeatNumber: repeatNumber, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_animationMoveX(moveValue: Any?,
                           duration: TimeInterval = 2.0,
                           delay: TimeInterval = 0,
                           repeatNumber: Float = 1,
                           removedOnCompletion: Bool = false,
                           option: CAMediaTimingFunctionName = .default) {
        ld_baseBasicAnimation(keyPath: "transform.translation.x", startValue: position, endValue: moveValue, duration: duration, delay: delay, repeatNumber: repeatNumber, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_animationMoveY(moveValue: Any?,
                           duration: TimeInterval = 2.0,
                           delay: TimeInterval = 0,
                           repeatNumber: Float = 1,
                           removedOnCompletion: Bool = false,
                           option: CAMediaTimingFunctionName = .default) {
        ld_baseBasicAnimation(keyPath: "transform.translation.y", startValue: position, endValue: moveValue, duration: duration, delay: delay, repeatNumber: repeatNumber, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_animationCornerRadius(cornerRadius: Any?,
                                  duration: TimeInterval = 2.0,
                                  delay: TimeInterval = 0,
                                  repeatNumber: Float = 1,
                                  removedOnCompletion: Bool = false,
                                  option: CAMediaTimingFunctionName = .default) {
        ld_baseBasicAnimation(keyPath: "cornerRadius", startValue: position, endValue: cornerRadius, duration: duration, delay: delay, repeatNumber: repeatNumber, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_animationScale(scaleValue: Any?,
                           duration: TimeInterval = 2.0,
                           delay: TimeInterval = 0,
                           repeatNumber: Float = 1,
                           removedOnCompletion: Bool = true,
                           option: CAMediaTimingFunctionName = .default) {
        ld_baseBasicAnimation(keyPath: "transform.scale", startValue: 1, endValue: scaleValue, duration: duration, delay: delay, repeatNumber: repeatNumber, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_animationRotation(rotation: Any?,
                              duration: TimeInterval = 2.0,
                              delay: TimeInterval = 0,
                              repeatNumber: Float = 1,
                              removedOnCompletion: Bool = true,
                              option: CAMediaTimingFunctionName = .default) {
        ld_baseBasicAnimation(keyPath: "transform.rotation", startValue: nil, endValue: rotation, duration: duration, delay: delay, repeatNumber: repeatNumber, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_baseBasicAnimation(keyPath: String,
                               startValue: Any?,
                               endValue: Any?,
                               duration: TimeInterval = 2.0,
                               delay: TimeInterval = 0,
...
public extension CALayer {
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_addKeyframeAnimationPosition(values: [Any],
                                         keyTimes: [NSNumber]?,
                                         duration: TimeInterval = 2.0,
                                         delay: TimeInterval = 0,
                                         repeatNumber: Float = 1,
                                         removedOnCompletion: Bool = false,
                                         option: CAMediaTimingFunctionName = .default) {
        ld_baseKeyframeAnimation(keyPath: "position", values: values, keyTimes: keyTimes, duration: duration, delay: delay, repeatNumber: repeatNumber, path: nil, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_addKeyframeAnimationRotation(values: [Any] = [CGFloat(-5).ld_degrees, CGFloat(5).ld_degrees, CGFloat(-5).ld_degrees],
                                         keyTimes: [NSNumber]?,
                                         ...
                                         removedOnCompletion: Bool = true,
                                         option: CAMediaTimingFunctionName = .default) {
        ld_baseKeyframeAnimation(keyPath: "transform.rotation", values: values, keyTimes: keyTimes, duration: duration, delay: delay, repeatNumber: repeatNumber, path: nil, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_addKeyframeAnimationPositionBezierPath(path: CGPath?,
                                                   duration: TimeInterval = 2.0,
                                                   delay: TimeInterval = 0,
                                                   repeatNumber: Float = 1,
                                                   removedOnCompletion: Bool = false,
                                                   option: CAMediaTimingFunctionName = .default) {
        ld_baseKeyframeAnimation(keyPath: "position", values: nil, keyTimes: nil, duration: duration,  delay: delay, repeatNumber: repeatNumber, path: path, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_baseKeyframeAnimation(keyPath: String,
                                  values: [Any]?,
                                  keyTimes: [NSNumber]?,
                                  duration: TimeInterval = 2.0,
                                  delay: TimeInterval = 0,
...
public extension CALayer {
    ...
    ///   - subtype: 过渡动画的方向
    ///   - duration: 动画的时间
    func ld_addTransition(type: CATransitionType,
...
public extension CALayer {
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_addSpringAnimationBounds(toValue: Any?,
                                     delay: TimeInterval = 0,
                                     mass: CGFloat = 10.0,
                                     ...
                                     removedOnCompletion: Bool = false,
                                     option: CAMediaTimingFunctionName = .default) {
        ld_baseSpringAnimation(path: "bounds", toValue: toValue, mass: mass, stiffness: stiffness, damping: damping, initialVelocity: initialVelocity, repeatNumber: repeatNumber, removedOnCompletion: removedOnCompletion, option: option)
    }
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_baseSpringAnimation(path: String?,
                                toValue: Any?,
                                delay: TimeInterval = 0,
                                mass: CGFloat = 10.0,
                                stiffness: CGFloat = 5000,
...
public extension CALayer {
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func ld_baseAnimationGroup(animations: [CAAnimation]?,
                               duration: TimeInterval = 2.0,
                               delay: TimeInterval = 0,
                               repeatNumber: Float = 1,
                               removedOnCompletion: Bool = false,
