...
public extension UIImage{
    ...
    static func JQ_Bundle(_ name:String, resoure atResoure:String = "Icon")->UIImage?{
        let a = Bundle(for: LDTools.self).path(forResource: atResoure, ofType: "bundle")
    ...
    ///压缩图片，限制最大宽高
    func ld_resizeImage(maxWidth: CGFloat = 1280,maxHeight: CGFloat = 720) -> UIImage{
    ...
    /// 生成一张高斯模糊图
    func ld_blur(_ value:CGFloat)->UIImage{
    ...
    static func ld_image(from layer: CALayer, scale: CGFloat = 0.0) -> UIImage? {
    ...
    /// alpha: 透明度
    /// - Returns: newImage
    func ld_imageByApplayingAlpha(_ alpha: CGFloat) -> UIImage {
    ...
    /// 添加水印方法:添加文字
    func ld_waterMarkeText(waterMarkText:String, corner:WaterMarkCorner = .BottomLeft,
    ...
    //添加图片水印方法：添加图片
    func ld_waterMarkeImg(waterMarkImage:UIImage, corner:WaterMarkCorner = .BottomRight,
    ...
    /// - Parameter rect: 尺寸
    static func ld_createClarityImg(rect:CGRect,alpha:CGFloat = 0)->UIImage{
     ...
     */
    func ld_filter(name:String) -> UIImage?
    {
    ...
    /// 更改图片颜色
    func ld_imageWithTintColor(color : UIColor) -> UIImage{
    ...
    ///   - blendMode: 模型
    /// - Returns: 颜色
    func ld_imageWithTintColor(color: UIColor, blendMode: CGBlendMode) -> UIImage?{
    ...
    ///   - size: 图片尺寸
    /// - Returns: 返回对应的图片
    static func ld_image(color: UIColor, size: CGSize = CGSize(width: 1.0, height: 1.0)) -> UIImage? {
        return ld_image(color: color, size: size, corners: .allCorners, radius: 0)
    }
    ...
    ///   - round: 圆角大小
    /// - Returns: 返回对应的图片
    static func ld_image(color: UIColor, size: CGSize, corners: UIRectCorner, radius: CGFloat) -> UIImage? {
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    static func ld_gradient(_ hexsString: [String], size: CGSize = CGSize(width: 1, height: 1), locations:[CGFloat]? = nil, direction: GradientDirection = .horizontal) -> UIImage? {
        return ld_gradient(hexsString.map{ UIColor(hexStr: $0) }, size: size, locations: locations, direction: direction)
    }
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    static func ld_gradient(_ colors: [UIColor], size: CGSize = CGSize(width: 10, height: 10), locations:[CGFloat]? = nil, direction: GradientDirection = .horizontal) -> UIImage? {
        return ld_gradient(colors, size: size, radius: 0, locations: locations, direction: direction)
    }
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 带圆角的渐变的图片
    static func ld_gradient(_ colors: [UIColor],
    ...
    ///   - height: 高度
    /// - Returns: 条形码
    func ld_generateBarCode(messgae:NSString,width:CGFloat,height:CGFloat) -> UIImage {
    ...
    //重设颜色
    func ld_imageWithNewSize(size: CGSize) -> UIImage? {
    ...
    /// - Parameter crop: 裁剪区域
    /// - Returns: 剪裁后的图片
    func ld_cropWithCropRect( _ crop: CGRect) -> UIImage? {
    ...
    ///将图片裁剪成指定比例（多余部分自动删除）
    func ld_crop(ratio: CGFloat) -> UIImage {
        //计算最终尺寸
        var newSize:CGSize!
    ...
    ///将图片缩放成指定尺寸（多余部分自动删除）
    func ld_scaled(to newSize: CGSize) -> UIImage {
        //计算比例
