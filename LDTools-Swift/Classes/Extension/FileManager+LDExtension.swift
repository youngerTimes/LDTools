...
public extension FileManager {
    ...
    /// 获取Home的完整路径名
    /// - Returns: Home的完整路径名
    static func ld_homeDirectory() -> String {
        //获取程序的Home目录
    ...
    static func ld_DocumnetsDirectory() -> String {
    ...
    static func ld_LibraryDirectory() -> String {
        //获取程序的documentPaths目录
        //Library目录－方法1
    ...
    static func ld_CachesDirectory() -> String {
    ...
    static func ld_PreferencesDirectory() -> String {
    ...
    /// - Returns: Tmp的完整路径名
    static func ld_TmpDirectory() -> String {
        //方法1
...
public extension FileManager {
    ...
    // MARK: 移动或者拷贝的类型
    /// 移动或者拷贝的类型
    enum LD_MoveOrCopyType {
        case file
        case directory
    }
    /// 文件管理器
    static var ld_fileManager: FileManager {
    ...
    static func ld_createFolder(folderPath: String) -> (isSuccess: Bool, error: String) {
        ...
        do {
            ...
            try ld_fileManager.createDirectory(atPath: folderPath, withIntermediateDirectories: true, attributes: nil)
            return (true, "")
        } catch _ {
            return (false, "创建失败")
        }
    ...
    @discardableResult
    static func ld_removefolder(folderPath: String) -> (isSuccess: Bool, error: String) {
        let filePath = "\(folderPath)"
        guard ld_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在就不做什么操作了
            return (true, "")
        }
        // 文件存在进行删除
        do {
            try ld_fileManager.removeItem(atPath: filePath)
            return (true, "")
        } catch _ {
            return (false, "删除失败")
        }
    ...
    @discardableResult
    static func ld_createFile(filePath: String) -> (isSuccess: Bool, error: String) {
        guard ld_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径才会创建
            // withIntermediateDirectories 为 ture 表示路径中间如果有不存在的文件夹都会创建
            let createSuccess = ld_fileManager.createFile(atPath: filePath, contents: nil, attributes: nil)
    ...
    /// - Parameter filePath: 文件路径
    @discardableResult
    static func ld_removefile(filePath: String) -> (isSuccess: Bool, error: String) {
        guard ld_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径就不需要要移除
            return (true, "")
        }
    ...
    /// - Returns: 文件内容
    @discardableResult
    static func ld_readfile(filePath: String) -> String? {
        guard ld_judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径就不需要要移除
            return nil
        }
        let data = ld_fileManager.contents(atPath: filePath)
    ...
    /// - Returns: 写入的结果
    @discardableResult
    static func ld_writeToFile(writeType: LD_FileWriteType, content: Any, writePath: String) -> (isSuccess: Bool, error: String) {
        guard ld_judgeFileOrFolderExists(filePath: writePath) else {
            // 不存在的文件路径
    ...
    /// - Returns: 返回读取的内容
    @discardableResult
    static func ld_readFromFile(readType: LD_FileWriteType, readPath: String) -> (isSuccess: Bool, content: Any?, error: String) {
        guard ld_judgeFileOrFolderExists(filePath: readPath),  let readHandler =  FileHandle(forReadingAtPath: readPath) else {
            // 不存在的文件路径
        ...
        switch readType {
            ...
            case .ArrayType:
                ...
                return (true, String.ld_toArray(readString), "")
            case .DictionaryType:
                guard let readString = String(data: data, encoding: String.Encoding.utf8) else {
                    return (false, nil, "读取内容失败")
                }
    ...
    /// - Returns: 拷贝的结果
    @discardableResult
    static func ld_copyFile(type: LD_MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> (isSuccess: Bool, error: String) {
        // 1、先判断被拷贝路径是否存在
        guard ld_judgeFileOrFolderExists(filePath: fromeFilePath) else {
            return (false, "被拷贝的(文件夹/文件)路径不存在")
        }
        // 2、判断拷贝后的文件路径的前一个文件夹路径是否存在，不存在就进行创建
        let toFileFolderPath = ld_directoryAtPath(path: toFilePath)
        if !ld_judgeFileOrFolderExists(filePath: toFileFolderPath), type == .file ? !ld_createFile(filePath: toFilePath).isSuccess : !ld_createFolder(folderPath: toFileFolderPath).isSuccess {
            return (false, "拷贝后路径前一个文件夹不存在")
        }
        // 3、如果被拷贝的(文件夹/文件)已存在，先删除，否则拷贝不了
        if isOverwrite, ld_judgeFileOrFolderExists(filePath: toFilePath) {
            do {
                try ld_fileManager.removeItem(atPath: toFilePath)
            } catch _ {
                return (false, "拷贝失败")
            }
        }
        // 4、拷贝(文件夹/文件)
        do {
            try ld_fileManager.copyItem(atPath: fromeFilePath, toPath: toFilePath)
        } catch _ {
            return (false, "拷贝失败")
        }
        return (true, "success")
    ...
    ///   - toFile: 移动后的文件路径
    @discardableResult
    static func ld_moveFile(type: LD_MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> (isSuccess: Bool, error: String) {
        // 1、先判断被拷贝路径是否存在
        guard ld_judgeFileOrFolderExists(filePath: fromeFilePath) else {
        ...
        // 4、移动(文件夹/文件)
        do {
            try ld_fileManager.moveItem(atPath: fromeFilePath, toPath: toFilePath)
        } catch _ {
            return (false, "移动失败")
        }
        return (true, "success")
    ...
    /** 判断文件或文件夹是否存在*/
    static func ld_judgeFileOrFolderExists(filePath: String) -> Bool {
        let exist = ld_fileManager.fileExists(atPath: filePath)
    ...
    static func ld_directoryAtPath(path: String) -> String {
        return (path as NSString).deletingLastPathComponent
    }

    // MARK: 2.12、判断目录是否可读
    static func ld_judegeIsReadableFile(path: String) -> Bool {
        return ld_fileManager.isReadableFile(atPath: path)
    }

    // MARK: 2.13、判断目录是否可写
    static func ld_judegeIsWritableFile(path: String) -> Bool {
        return ld_fileManager.isReadableFile(atPath: path)
    }
    ...
    /// - Parameter path: 文件路径
    /// - Returns: 文件扩展类型
    static func ld_fileSuffixAtPath(path: String) -> String {
    ...
    ///   - suffix: 是否需要后缀，默认需要
    /// - Returns: 文件名称
    static func ld_fileName(path: String, suffix: Bool = true) -> String {
    ...
    static func ld_shallowSearchAllFiles(folderPath: String) -> Array<String>? {
        do {
            let contentsOfDirectoryArray = try ld_fileManager.contentsOfDirectory(atPath: folderPath)
    ...
    static func ld_getAllFileNames(folderPath: String) -> Array<String>? {
        // 查看文件夹是否存在，如果存在就直接读取，不存在就直接反空
        if (ld_judgeFileOrFolderExists(filePath: folderPath)) {
            guard let subPaths = ld_fileManager.subpaths(atPath: folderPath) else {
                return nil
            }
            return subPaths
        } else {
    ...
    static func ld_deepSearchAllFiles(folderPath: String) -> Array<Any>? {
        // 查看文件夹是否存在，如果存在就直接读取，不存在就直接反空
        if (ld_judgeFileOrFolderExists(filePath: folderPath)) {
            guard let contentsOfPathArray = ld_fileManager.enumerator(atPath: folderPath) else {
                return nil
            }
    ...
    /// - Returns: 单个文件或文件夹的大小
    static func ld_fileOrDirectorySingleSize(filePath: String) -> UInt64 {
        // 1、先判断文件路径是否存在
        guard ld_judgeFileOrFolderExists(filePath: filePath) else {
            return 0
        }
        // 2、读取文件大小
        do {
            let fileAttributes = try ld_fileManager.attributesOfItem(atPath: filePath)
    ...
    /// - Parameter path: (文件夹/文件) 的路径
    /// - Returns: (文件夹/文件) 的大小
    static func ld_fileOrDirectorySize(path: String) -> String {
        if path.count == 0, !ld_fileManager.fileExists(atPath: path) {
            return "0MB"
        }
        // (文件夹/文件) 的实际大小
        var fileSize: UInt64 = 0
        do {
            let files = try ld_fileManager.contentsOfDirectory(atPath: path)
            for file in files {
                let path = path + "/\(file)"
                fileSize = fileSize + ld_fileOrDirectorySingleSize(filePath: path)
            }
        } catch {
            fileSize = fileSize + ld_fileOrDirectorySingleSize(filePath: path)
        }
        // 转换后的大小 ["bytes", "KB", "MB", "GB", "TB", "PB",  "EB",  "ZB", "YB"]
        return ld_covertUInt64ToString(with: fileSize)
    }

    /// 是否是文件夹
    /// - Parameter path: 路径
    /// - Returns: 返回判断状态
    static func ld_isDirectory(path:String)->Bool{
        do {
            let attribute = try ld_fileManager.attributesOfItem(atPath: path)
    ...
    /// - Returns: (文件夹/文件)属性集合
    @discardableResult
    static func ld_fileAttributes(path: String) -> ([FileAttributeKey : Any]?) {
        do {
            let attributes = try ld_fileManager.attributesOfItem(atPath: path)
            /*
...
extension FileManager {
    ...
    /// - Returns: 转换后的文件大小
    fileprivate static func ld_covertUInt64ToString(with size: UInt64) -> String {
