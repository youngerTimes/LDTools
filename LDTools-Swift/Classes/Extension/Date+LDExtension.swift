...
public extension Date{
    ...
    /// 当月第一天是星期几
    static func ld_weekDayInCurrentMonth() ->Int {
        //星期和数字一一对应 星期日：7
    ...
    static func ld_solarToLunar(year: Int, month: Int, day: Int) -> String {
        //初始化公历日历
    ...
    ///农历转公历
    static func ld_convert(year: Int, month: Int, day: Int) -> (Int, Int, Int){
    ...
    /// 计算两个时间段的间隔
    static func ld_CalBySet(startDate:Date,endDate:Date)->(year:Int,month:Int,day:Int,hour:Int,minute:Int){
        //格式化成：秒归零
        let formatStartDate = Date.ld_format(year: startDate.year, month: startDate.month, day: startDate.day,hour:startDate.hour,minute: startDate.minute)
        let formatEndDate = Date.ld_format(year: endDate.year, month: endDate.month, day: endDate.day,hour:endDate.hour,minute: endDate.minute)
    ...
    /// 计算两个时间段的间隔
    /// - Returns: 返回天数
    static func ld_CalByDays(startDate:Date,endDate:Date)->Int{
        //格式化成：时分秒归零
        let formatStartDate = Date.ld_format(year: startDate.year, month: startDate.month, day: startDate.day,hour:0,minute: 0)
        let formatEndDate = Date.ld_format(year: endDate.year, month: endDate.month, day: endDate.day,hour:0,minute: 0)
    ...
    /// 计算两个时间段的间隔【完成了格式化操作】
    static func ld_CalByStr(startDate:Date,endDate:Date)->String{
        var str = ""
        let calendar = Calendar(identifier: .gregorian)
        let set = Set(arrayLiteral: Calendar.Component.day,Calendar.Component.year,Calendar.Component.month,Calendar.Component.hour,Calendar.Component.minute)
    ...
    ///时间戳转换字符串，yyyy-MM-dd
    static func ld_TheTimeStampConversionMinutes(_ time:TimeInterval, _ formatter:String) -> String {
    ...
    ///时间字符串转时间戳
    static func ld_StringToTimeInterval(_ dateStr:String, _ formatter:String) -> TimeInterval {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = formatter
    ...
    static func ld_TimeBeforeInfoToString(_ timeIntrval:TimeInterval) -> String {
        let dateFormatter = DateFormatter()
        ...
        if year > 0 {
            return self.ld_TheTimeStampConversionMinutes(timeIntrval, "yyyy-MM-dd")
        }else if month > 0 {
            return self.ld_TheTimeStampConversionMinutes(timeIntrval, "MM-dd")
        }else if day > 0 {
            if day > 7 {
                return "一周前"
            }else {
            ...
            if ld_isToday(timeterval: timeIntrval) {
                return self.ld_TheTimeStampConversionMinutes(timeIntrval, "HH:mm")
            }else {
                return self.ld_TheTimeStampConversionMinutes(timeIntrval, "昨天 HH:mm")
            }
        }else if minute > 0 {
            return "\(minute)分钟前"
        }else {
    ...
    ///将时间戳传入，返回距离今天有多少小时
    static func ld_timeBeforHours(_ timeIntrval:TimeInterval) -> Int {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
        let nowTime = NSDate().timeIntervalSince1970
    ...
    ///判断一个时间戳是否为今天
    static func ld_isToday(timeterval:TimeInterval) -> Bool {
        return Calendar.current.isDateInToday(Date(timeIntervalSince1970: timeterval))
    }
    
    /// 判断一个时间戳是否为昨天
    static func ld_isYesterday(timeterval:TimeInterval) -> Bool {
    ...
    /// - Parameter year: 设定年
    /// - Parameter month: 设定月
    static func ld_getDays(_ year:Int,_ month:Int)->Int{
    ...
    /// 给定年月日，得到Date的时间类型
    static func ld_format(year:NSInteger,month:NSInteger,day:NSInteger,hour:NSInteger = 0,minute:NSInteger = 0)->Date?{
        let calendar = Calendar(identifier: .gregorian)
    ...
    /// 根据时间戳，返回年龄
    static func ld_calculateAge(time: TimeInterval) -> NSInteger {
    ...
    ///获取上一个周
    static func ld_lastWeek(_ date:Date)->Date{
        return date.addingTimeInterval(-7 * 24 * 3600)
    }
    
    ///获取下一个周
    static func ld_nextWeek(_ date:Date)->Date{
        return date.addingTimeInterval(7 * 24 * 3600)
    }
    
    ///根据时间获取一个周的时间表
    static func ld_weekDates(_ date:Date)->[Date]{
        var dates = [Date]()
        let timeInterval = 24 * 3600
        for week in 1...7{
            let date = date.ld_startOfWeek.addingTimeInterval(TimeInterval(week * timeInterval + 8 * 3600))
            dates.append(date)
        }
        return dates
    }
   ...
   static var ld_stamp:String{
    ...
    var ld_endOfWeek: Date {
        let calendar = NSCalendar.current
        var components = DateComponents()
        components.day = 6
        return calendar.date(byAdding: components, to:ld_startOfWeek)!
    }
    
    ///获取上一周的时间表
    var ld_lastWeekDates:[Date]{
        let temp_lastDate = self.addingTimeInterval(-7 * 24 * 3600)
        return Date.ld_weekDates(temp_lastDate)
    }
    
    ///获取上一周的时间表
    var ld_nextWeekDates:[Date]{
        let temp_nextDate = self.addingTimeInterval(7 * 24 * 3600)
        return Date.ld_weekDates(temp_nextDate)
    }
    
    ///获取本周的时间表
    var ld_currentWeekDates:[Date]{
        return Date.ld_weekDates(self)
    }
    
    ///获取上一个周
    var ld_lastWeekDate:Date{
        return self.addingTimeInterval(-7 * 24 * 3600)
    }
    
    ///获取下一个周
    var ld_nextWeekDate:Date{
        return self.addingTimeInterval(7 * 24 * 3600)
    }
    
    //该时间所在周的最后一天日期（2017年12月23日 23:59:59）
    var ld_endTimeOfWeek: Date {
        let calendar = NSCalendar.current
        var components = DateComponents()
        components.day = 7
        components.second = -1
        return calendar.date(byAdding: components, to: self.ld_startOfWeek)!
    }

    /// 根据描述获取时分秒
    /// - Parameter interval: 秒数
    /// - Returns: 返回元祖
    static func LD_secondToDate(interval:Int)->(hour:Int,minute:Int,second:Int){
    ...
    /// 时间转换为时分秒
     static func ld_transToHourMinSec(time: Int,retract:Bool = true) -> String{
    ...
    ///将秒数转换为时分秒的字符串
    @available(*,deprecated,message: "废弃:使用 LD_secondToDate")
    func ld_transSingleToHourMinSec(time: Float,type:JQDateType = .HMS) -> String{
    ...
    //是否是闰年
    func ld_leapYear(_ year:Int)->Bool{
        if year % 4 == 0 && year % 100 != 0 {
            return true
        }
        return false
    ...
    /// 获取指定年份
    func ld_allYears(_ startYear:Int = 1900)->Array<Int>{
        var years = [Int]()
        for i in startYear..<ld_nowDay() { years.append(i) }
        return years
    }
    
    /// 全部月份
    func ld_allMonths()->Array<Int>{
        var months = [Int]()
    ...
    /// 判断两个日期是否相等
    func ld_equal(_ date:Date)->Bool{
        if self.ld_nowYear() == date.ld_nowYear() && self.ld_nowMonth() == date.ld_nowMonth() && self.ld_nowDay() == date.ld_nowDay(){
            return true
        }
        return false
    }
    ...
    /// 当天周数
    func ld_nowWeekDay()->(weekName:String,index:NSInteger){
        let calendar = Calendar(identifier: .gregorian)
    ...
    /// date实例格式化时间
    func ld_format(_ formatter:String)->String{
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = formatter
    ...
    func ld_formatInfo(_ formatter:String = "MM:DD")->String{
        return ""
    }
    
    ///时间戳格式格式化时间
    static func ld_format(_ time:TimeInterval,formatter:String)->String{
        let detaildate = NSDate(timeIntervalSince1970: time)
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = formatter
        return dateFormatter.string(from: detaildate as Date)
    ...
    ///时间戳格式格式化时间
    static func ld_formatDate(_ time:TimeInterval,formatter:String)->Date{
