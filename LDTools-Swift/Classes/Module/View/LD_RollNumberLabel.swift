...
public class LD_RollNumberLabel: UIView {
    ...
    public var valueNumber:NSNumber = NSNumber(floatLiteral: 0){
        didSet{
            if self.valueNumber.stringValue.contains(".") {
                rawText =  financeType ? valueNumber.doubleValue.ld_fuCoin(financeStyle, format: format) : "\(defaultNumber!.doubleValue)"
            }else{
                rawText = financeType ? valueNumber.intValue.ld_fuCoin(financeStyle, format: format) : "\(valueNumber.intValue)"
            }
            layout()
        }
    }
    ...
    public convenience init(_ defaultNumber:NSNumber = NSNumber(value: 0),financeType:Bool = false,financeStyle:NumberFormatter.Style = .decimal,format:String = ",###.##") {
        ...
        if defaultNumber.stringValue.contains(".") {
            rawText =  financeType ? defaultNumber.doubleValue.ld_fuCoin(financeStyle, format: format) : "\(defaultNumber.doubleValue)"
        }else{
            rawText = financeType ? defaultNumber.intValue.ld_fuCoin(financeStyle, format: format) : "\(defaultNumber.intValue)"
        }
    }
    ...
    private func layout(){
        ...
        for (index,item) in items.enumerated() {
            ...
            if item.contains(".") || item.contains(",") {
                ...
                scrollView.contentInset = UIEdgeInsets(top: -6 * LD_RateW, left: 0, bottom: 0, right: 0)
    ...
    private func setUI(){
        if valueNumber.stringValue.contains(".") {
            rawText =  financeType ? valueNumber.doubleValue.ld_fuCoin(financeStyle, format: format) : "\(valueNumber.doubleValue)"
        }else{
            rawText = financeType ? valueNumber.intValue.ld_fuCoin(financeStyle, format: format) : "\(valueNumber.intValue)"
        }
    }
