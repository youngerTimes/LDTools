...
/// 线性进度条
public class LDProgressView: UIView {
    ...
    public var pradientType:LDPradientType = .LeftToRight {//渐变方向
        didSet{
    ...
    //xib创建需要先指定此类型
    public var style:LDProgressStyle = .none{
        didSet{
        ...
        didSet{
            if animation {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now()+0.5) {
                    ...
                    weakSelf.sizeAni.toValue = weakSelf.progress * weakSelf.ld_width
                    weakSelf.lastWidth = weakSelf.progress * weakSelf.ld_width

                    weakSelf.positionAni.toValue = weakSelf.progress * weakSelf.ld_width / 2
    ...
    ///   - colors: 渐变颜色组
    ///   - pradientType: 渐变方向
    public convenience init(corner:Bool = false,animation:Bool = true,duration:CGFloat = 1.5,colors:[CGColor],pradientType:LDPradientType){
    ...
    private func setUI(){
        ...
        hintL.snp.makeConstraints { (make) in
            make.edges.equalToSuperview().inset(UIEdgeInsets(top: 0, left: 20 * LD_RateW, bottom: 0, right: 20 * LD_RateW))
        }
    }
